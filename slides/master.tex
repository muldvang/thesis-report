...
\begin{frame}
  ...
  \onslide<+->{
    Backtracking using argmax table.
  }
...
\begin{frame}
  ...
  \begin{center}
    ...
    \begin{tabular}{rccccccccccccccccc}
      \only<1> {$\delta_{T} =$ & $C_{\rain}$ & $\odot$ & $C_{\cloud}$ & $\odot$ & $C_{\sun}$ & $\odot$ & $C_{\cloud}$ & $\odot$ & $C_{\sun}$ & $\odot$ & $C_{\rain}$ & $\odot$ & $C_{\cloud}$ & $\odot$ & $C_{\sun}$ & $\odot$ & $C_1$ \\}%
  ...
  \end{center}
  \vfill{}
  \fxwarning{Backtracking.}
  \fxwarning{Running time.}
\end{frame}
