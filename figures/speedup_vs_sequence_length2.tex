\tikzsetnextfilename{speedup_vs_sequence_length2}
\begin{tikzpicture}[trim axis left,trim axis right]
  \begin{axis}[
    xlabel=Sequence length $T$,
    ylabel=Speedup factor,
    xmode=log,
    ]
    \addplot[only marks, color=colorbrewer1, error bars, y dir=both, y explicit]
    table[x=T, y=zipHMMlib_uncompressed_running_ratio, y error=zipHMMlib_uncompressed_running_ratio_std]
    {plot_data/viterbi_sequence_transformed.data};

    \addplot[only marks, color=colorbrewer2, error bars, y dir=both, y explicit]
    table[x=T, y=zipHMMlib_uncompressed_path_running_ratio, y error=zipHMMlib_uncompressed_path_running_ratio_std]
    {plot_data/viterbi_sequence_transformed.data};

    \addplot[only marks, color=colorbrewer3, error bars, y dir=both, y explicit]
    table[x=T, y=zipHMMlib_running_ratio, y error=zipHMMlib_running_ratio_std]
    {plot_data/viterbi_sequence_transformed.data};

    \addplot[only marks, color=colorbrewer4, error bars, y dir=both, y explicit]
    table[x=T, y=zipHMMlib_path_running_ratio, y error=zipHMMlib_path_running_ratio_std]
    {plot_data/viterbi_sequence_transformed.data};

    \addplot[only marks, color=colorbrewer5, error bars, y dir=both, y explicit]
    table[x=T, y=zipHMMlib_path_memory_running_ratio, y error=zipHMMlib_path_memory_running_ratio_std]
    {plot_data/viterbi_sequence_transformed.data};

    \legend{Uncompressed, Uncompressed Path, Compressed, Compressed Path,
      Compressed Path Space Saving}
  \end{axis}
\end{tikzpicture}

%%% Local Variables:
%%% mode: latex
%%% TeX-master: "../master"
%%% End:
