...
\item \fxnote{Make a new kind of compression stopping criterion. The
    compression should be stopped when it does not become faster, only due to
    the computation of C matrices and not since compression takes time.}
\end{itemize}

In most cases the added algorithms to zipHMM perform better than the classical
algorithms. The algorithms should not be used with large models, but this is
also normally preferred as the classical algorithms have a running time
...
useful. The classical algorithms are very simple and thus it can also hard to
