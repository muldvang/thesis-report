...
probabilities for each sequence is returned.  This has also been implemented for
...
has to specify the alphabet size since the sequence might not contain all
...
to the disk by \texttt{HMM\_Suite.write\_to\_directory(\dots)}. It can then be
...
observation sequence $Y_{1:T}$, compressed should be disabled. This can be achieved by
...
For the indexed posterior decoding compression can be enabled. The
